  ...
  loadReservations(): void {
    ...
    this.reservationService.getUserReservations(currentUser.userId).subscribe({
      ...
      error: (error) => {
        ...
        this.notificationService.error('Rezervasyonlar yüklenirken bir hata oluştu!');
        this.isLoading = false;
      }
    });
  }
    ...
    this.notificationService.confirm(
      ...
      () => {
        this.reservationService.cancelReservation(reservationId).subscribe({
          next: () => {
            this.notificationService.success('Rezervasyon başarıyla iptal edildi!');
            this.loadReservations();
          },
          error: (error) => {
            this.notificationService.error(error.error?.message || 'Rezervasyon iptal edilemedi!');
          }
        });
      }
    );
